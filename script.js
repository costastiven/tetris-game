// Define el tamaño de cada espacio de la cuadrícula
const gridSpace = 30;
...
function setup() {
    ...
    // Crea una nueva pieza que cae
    fallingPiece = new PlayPiece();
    fallingPiece.resetPiece();

    // Establezca la fuente para el texto
    textFont('Ubuntu');
}

// Función de dibujo llamada repetidamente
function draw() {
    ...
    // Dibuja el panel de información del lado derecho
    fill(25);
    noStroke();
    rect(gameEdgeRight, 0, 150, height);

    // Dibuja el panel de información del lado izquierdo
    rect(0, 0, gameEdgeLeft, height);

    // Dibuje el rectángulo de puntuación
    fill(colorBackground);
    rect(450, 80, 150, 70);

    // Dibuja el siguiente trozo rectángulo
    rect(460, 405, 130, 130, 5, 5);

    // Dibuja el rectángulo de nivel
    rect(460, 210, 130, 60, 5, 5);

    // Dibuja las líneas del rectángulo
    rect(460, 280, 130, 60, 5, 5);

    // Dibuje las líneas de puntuación
    fill(colorLight);
    rect(450, 85, 150, 20);
    rect(450, 110, 150, 4);
    rect(450, 140, 150, 4);

    // Dibuja el banner de puntuación
    fill(colorBackground);
    rect(460, 60, 130, 35, 5, 5);

    // Dibuja el rectángulo interior del cartel de puntuación
    strokeWeight(3);
    noFill();
    stroke(colorLight);
    rect(465, 65, 120, 25, 5, 5);

    // Dibuja la siguiente pieza del rectángulo interior
    stroke(colorLight);
    rect(465, 410, 120, 120, 5, 5);

    // Dibuja el rectángulo interior del nivel
    rect(465, 215, 120, 50, 5, 5);

    // Dibuja las líneas del rectángulo interior
    rect(465, 285, 120, 50, 5, 5);

    // Dibuja las etiquetas de información
    fill(25);
    noStroke();
    textSize(24);
    textAlign(CENTER);
    ...
    // Acelera la caída de la pieza si se presiona la flecha hacia abajo
    ...
    // Procesar las rejillas de la red
    ...
    textSize(14);
    text("Controles:\n↑\n← ↓ →\n", 75, 155);
    text("Iquierda y Derecha:\nMover lado a lado", 75, 230);
    text("Arriba:\nRotar", 75, 280);
...
class PlayPiece {
    ...
    //Agrega un desplazamiento a la posición de la pieza actual
    addPos(x, y) {
        this.pos.x += x;
        this.pos.y += y;
    ...
    // Comprueba si habrá una colisión en el futuro
...
// Clase para cada cuadrado de una pieza
class Square {
...
// Generar un número pseudoaleatorio para la siguiente pieza
...
//Analiza la cuadrícula y borra líneas si es necesario
...
// Comprueba si hay líneas completas en la cuadrícula
function checkLines() {
    for (let y = 0; y < height; y += gridSpace) {
        ...
        if (count === 10) {
            //Elimina las piezas en esta coordenada y
            gridPieces = gridPieces.filter(piece => piece.pos.y !== y);
            // Mueve hacia abajo las piezas que están por encima de esta coordenada y
            for (let i = 0; i < gridPieces.length; i++) {
                if (gridPieces[i].pos.y < y) {
...
// Clase para la grilla de la red
class Worker {
